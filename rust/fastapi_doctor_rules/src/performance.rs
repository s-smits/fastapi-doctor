...
pub(crate) fn collect_regex_in_loop_issues(module: &ModuleIndex, suite: &ast::Suite) -> Vec<Issue> {
    ...
    walk_for_regex_in_loop(suite, 0, module, &re_funcs, &mut issues, &mut seen_lines);
    issues
}
...
pub(crate) fn collect_sequential_awaits_issues(
    ...
    fn is_await_assign(stmt: &Stmt) -> Option<(&ast::ExprCall, HashSet<String>)> {
    ...
    fn shared_session_name(call: &ast::ExprCall, session_hints: &HashSet<&str>) -> Option<String> {
        ...
        if let Some(Expr::Name(first_arg)) = call.args.first() {
            if session_hints.contains(first_arg.id.to_ascii_lowercase().as_str()) {
                return Some(first_arg.id.to_string());
            }
        }
        None
    }
    ...
    for function in all_functions(suite) {
        ...
        while i < body.len().saturating_sub(1) {
            ...
            while i < body.len() {
                ...
                walk_expr_tree(&Expr::Call(call.clone()), &mut |expr| {
                    if let Expr::Name(n) = expr {
                ...
                run_sessions.push(shared_session_name(call, &session_hints));
