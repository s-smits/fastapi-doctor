...
pub(crate) const AWAITABLE_RETURN_CALLS: &[&str] = &["create_task", "ensure_future", "gather", "shield"];
pub(crate) const SYNC_HTTP_ATTRS: &[&str] = &["get", "post", "put", "patch", "delete", "head", "request"];
pub(crate) const ASYNC_HELPER_MAX_DEPTH: usize = 5;
...
#[derive(Default)]
pub(crate) struct FunctionBodyCollector {
    pub(crate) direct_calls: Vec<CallSite>,
    pub(crate) await_calls: Vec<CallSite>,
    pub(crate) async_for_calls: Vec<CallSite>,
    pub(crate) async_with_calls: Vec<CallSite>,
    pub(crate) has_async_constructs: bool,
    pub(crate) has_async_for_or_with: bool,
    pub(crate) is_generator: bool,
    pub(crate) returns_awaitable: bool,
}
...
pub(crate) struct FunctionContext {
    ...
    pub(crate) has_async_constructs: bool,
    pub(crate) is_generator: bool,
    pub(crate) is_sync_context_manager: bool,
    pub(crate) returns_awaitable: bool,
    pub(crate) line: usize,
    pub(crate) direct_calls: Vec<CallSite>,
    pub(crate) await_calls: Vec<CallSite>,
...
pub(crate) struct FunctionIndex {
    ...
    pub(crate) by_method: HashMap<(String, String), usize>,
}

impl FunctionIndex {
    pub(crate) fn from_suite(module: &ModuleIndex, suite: &ast::Suite) -> Self {
    ...
    pub(crate) fn get_context(&self, qualname: &str) -> Option<&FunctionContext> {
...
pub(crate) fn build_function_context(
    module: &ModuleIndex,
...
pub(crate) fn build_async_function_context(
    module: &ModuleIndex,
...
pub(crate) fn build_context_common(
    module: &ModuleIndex,
    name: &str,
...
pub(crate) fn looks_like_route_handler(module: &ModuleIndex, decorator: &Expr) -> bool {
    let source = module.source_slice(decorator.range()).to_ascii_lowercase();
    source.contains("router") || source.contains("app")
}

pub(crate) fn looks_like_context_manager(decorator: &Expr, async_only: bool) -> bool {
...
pub(crate) fn decorator_name(decorator: &Expr) -> Option<String> {
    match decorator {
...
pub(crate) fn call_site(module: &ModuleIndex, call: &ast::ExprCall, owner_class: Option<&str>) -> CallSite {
...
pub(crate) fn returns_awaitable_call(expr: &Expr) -> bool {
...
pub(crate) struct FunctionSpan<'a> {
    pub(crate) name: &'a str,
    pub(crate) is_async: bool,
    pub(crate) range: rustpython_parser::ast::text_size::TextRange,
    pub(crate) body: &'a [Stmt],
    pub(crate) defaults: Vec<&'a Expr>,
}

pub(crate) fn all_functions<'a>(suite: &'a [Stmt]) -> Vec<FunctionSpan<'a>> {
    let mut functions = Vec::new();
    for stmt in suite {
...
pub(crate) fn collect_functions<'a>(stmt: &'a Stmt, functions: &mut Vec<FunctionSpan<'a>>) {
    match stmt {
        Stmt::FunctionDef(node) => {
...
pub(crate) fn walk_suite_stmts<'a>(suite: &'a [Stmt], visit: &mut impl FnMut(&'a Stmt)) {
    for stmt in suite {
        walk_stmt_tree(stmt, visit);
    }
}

pub(crate) fn walk_stmt_tree<'a>(stmt: &'a Stmt, visit: &mut impl FnMut(&'a Stmt)) {
    visit(stmt);
    match stmt {
        Stmt::FunctionDef(node) => {
            for inner in &node.body {
...
pub(crate) fn walk_suite_exprs<'a>(suite: &'a [Stmt], visit: &mut impl FnMut(&'a Expr)) {
    for stmt in suite {
        walk_stmt_exprs(stmt, visit);
    }
}

pub(crate) fn walk_stmt_exprs<'a>(stmt: &'a Stmt, visit: &mut impl FnMut(&'a Expr)) {
    match stmt {
        Stmt::FunctionDef(node) => {
...
pub(crate) fn walk_expr_tree<'a>(expr: &'a Expr, visit: &mut impl FnMut(&'a Expr)) {
    visit(expr);
    match expr {
        Expr::BoolOp(node) => {
            for value in &node.values {
...
pub(crate) fn function_default_exprs<'a>(args: &'a ast::Arguments) -> Vec<&'a Expr> {
    args.defaults()
        .chain(
            args.kwonlyargs
                .iter()
...
pub(crate) fn module_has_async_def(suite: &[Stmt]) -> bool {
    all_functions(suite)
        .iter()
        .any(|function| function.is_async)
}

pub(crate) fn name_main_line_ranges(module: &ModuleIndex, suite: &[Stmt]) -> Vec<(usize, usize)> {
...
pub(crate) fn suite_imports_threading_lock(suite: &[Stmt]) -> bool {
    let mut found = false;
    walk_suite_stmts(suite, &mut |stmt| {
...
pub(crate) fn max_nesting_depth(body: &[Stmt]) -> usize {
    let mut max_depth = 0;
    for stmt in body {
...
pub(crate) fn max_nesting_depth_stmt(stmt: &Stmt, depth: usize) -> usize {
...
pub(crate) fn max_nesting_in_block(body: &[Stmt], depth: usize) -> usize {
    let mut max_depth = depth;
    for stmt in body {
